<?php
   ...
   $today=date("d-m-Y");
   $next= date('d-m-Y', strtotime($today. ' -1 days'));
   $pdf = new FPDF();
   $pdf->AddPage();
   $pdf->Image("img/white.jpeg", 155, 2,50,30);
   $pdf->SetFont("Arial","B", 16);
   $pdf->SetTextColor(0, 0, 0);
   $pdf->Cell(30,20,"Date:$next","0","1","C");
   $pdf->SetFont("Arial","B", 16);
   ...
   $pdf->SetTextColor(0,0,0);
    ...
    $result=$mysqli->query("SELECT * FROM report where Report_date = curdate()-3") or die($mysqli->error);

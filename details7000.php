...
<center><h2><u>7000 Contribution results</u> </h2></center>
<?php
$mysqli = new mysqli('localhost', 'root', '', 'meetings') or die(mysqli_error($mysqli));
$result = $mysqli->query("SELECT member_id, contribution_benefit, contribution_paidfine, contribution_fine + contribution_amount as damn, firstname,contribution_payment,contribution_amount, beneficiary FROM member right join beneficiary on member_id=benefit_id right join Contribution on contribution_id=member_id where contribution_amount=7000 order by contribution asc ") or die(mysqli_error($mysqli));
$result2 = $mysqli->query("SELECT  sum(contribution_paidfine) as san, sum(contribution_payment) as form,sum(contribution_amount) as contrib FROM Contribution where contribution_amount=7000") or die(mysqli_error($mysqli));
/* pre_r($result); */
?>
<center>
<div class="container">

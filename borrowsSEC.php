...
$result = $mysqli->query("SELECT id, ceiling(saving_amount+saving_amount * savings_interest) as savings, cast(DOP as date) as hier,avalist,bname,amount,refunded_amount, ceiling((amount+amount * 10/100) - (saving_amount+saving_amount * savings_interest)-refunded_amount) as amountsert,  (amount+amount * 10/100) - ceiling((saving_amount+saving_amount * savings_interest)) as amountsend,  amount+amount * 10/100 as amountpay, cast(ADDDATE(DOP, interval 3 month) as date) as dateline FROM borrows left join avance on borrows.bname=avance.avance_name inner join benefiter on benefiter.benefiter_id=borrows.id left join savings on savings.savings_id=borrows.id") or die(mysqli_error($mysqli));
